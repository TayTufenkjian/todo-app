...
function showListItems(listID) {
  ...
  const deleteFunction = () => {
    ...
    const confirmDelete = confirm(
      "Do you really want to delete this list? You will also delete all to-do items in the list."
    );
    if (confirmDelete) {
      deleteList(listID);
      showLists();
  ...
  const btnDeleteList = createButton(
    "Delete list",
    deleteFunction,
    "secondary"
  );
...
function listenForAddNewList() {
  document.getElementById("add-list").addEventListener("click", () => {
    ...
    const listNameField = createFormField(
      "text",
      "list-name",
      "list-name",
      "Name of list",
